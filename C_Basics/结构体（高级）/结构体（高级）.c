...
// 
// 声明和结构体类似，两个不同：
// 1.成员只能是整型，且最好只有char、short、int、long中的一个，有无符号不做要求
...
//位段的内存分配
// 1.段位的空间是按照需要以一个位段（4个字节或1个字节）来开辟的
...
// 4.一个位段剩余的bit容不下下一个成员，对剩余的bit是舍弃还是利用不确定
// 
// 对于4，vs环境下剩余的bit舍弃，再开一个位段
//
...
// 
// 每个成员的对齐数 == 编译器默认的对齐数 与 该成员大小 的较小值
// vs默认对齐数为8，gcc无默认对齐数
// 
// 3.结构体总大小为，成员中最大对齐数的整数倍
// 4.结构体嵌套
//   内层结构体的对齐数是其本身成员的最大对齐数，其余同上
// 
// 意义
